...
def title_bar(idx, name, emoji, vibe, accent='#fbbf24', dim='#5c3f1e'):
  ...
  <div class="nav">
    <a href="cockpit-lab_v5.html" style="color:{accent};border-color:{dim};">⚙ Gallery</a>
    <a href="cockpit-lab_v5_{prev_idx}.html" style="color:{accent};border-color:{dim};">‹ Prev</a>
    <a href="cockpit-lab_v5_{next_idx}.html" style="color:{accent};border-color:{dim};">Next ›</a>
  </div>
</div>'''
...
for i, maker in enumerate(makers, 1):
    out = os.path.join(OUT, f'cockpit-lab_v5_{i}.html')
    with open(out, 'w', encoding='utf-8') as f:
        f.write(maker())
    emoji, name, _ = labels[i-1]
    print(f'  + cockpit-lab_v5_{i}.html  ({emoji} {name})')
...
<div class="nav">
  ...
  <a href="cockpit-lab_v4.html">v4</a>
...
for i, (emoji, name, vibe) in enumerate(labels, 1):
    ...
    gallery += f'''  <a class="{klass}" href="cockpit-lab_v5_{i}.html" style="{op}">
    <div class="emoji">{emoji}</div>
    <div class="num">cockpit-lab_v5_{i}.html</div>
    <h3>{name}</h3>
    <div class="vibe">{vibe}</div>
...
'''
with open(os.path.join(OUT, 'cockpit-lab_v5.html'), 'w', encoding='utf-8') as f:
    f.write(gallery)
print('  + cockpit-lab_v5.html  (gallery)')
print('Done.')
